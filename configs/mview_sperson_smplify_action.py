type = 'MultiViewSinglePersonSMPLEstimator'
work_dir = './temp'
verbose = True
logger = None
bbox_detector = None
kps2d_estimator = None
triangulator = None
smplify = dict(
    type='SMPLifyX',
    verbose=verbose,
    info_level='stage',
    n_epochs=1,
    use_one_betas_per_video=True,
    hooks=[
        dict(type='SMPLifyVerboseHook'),
    ],
    grad_clip=10,
    logger=logger,
    body_model=dict(
        type='SMPLX',
        gender='neutral',
        num_betas=10,
        keypoint_convention='smplx', 
        model_path='human_models/human_model_files/smplx',
        batch_size=1,
        use_face_contour=True,
        use_pca=False,
        num_pca_comps=24,
        flat_hand_mean=False,
        logger=logger),
    optimizer=dict(
        type='LBFGS', max_iter=20, lr=1.0, line_search_fn='strong_wolfe'),
    # optimizer=dict(
    #     individual_optimizer=True,
    #     body_pose_optimizer=dict(
    #     type='LBFGS', max_iter=20, lr=0.1, line_search_fn='strong_wolfe'),
    #     betas_optimizer=dict(
    #     type='LBFGS', max_iter=20, lr=1.0, line_search_fn='strong_wolfe'),
    #     default_optimizer=dict(
    #     type='LBFGS', max_iter=20, lr=1.0, line_search_fn='strong_wolfe')),
    ignore_keypoints=[
        'right_smalltoe', 'right_bigtoe', 'left_smalltoe', 'left_bigtoe'
    ],
    handlers=[
        dict(
            handler_key='keypoints3d_mse',
            type='Keypoint3dMSEHandler',
            mse_loss=dict(
                type='KeypointMSELoss',
                loss_weight=10.0,
                reduction='sum',
                sigma=100),
            logger=logger),
        dict(
            handler_key='keypoints2d_mse',
            type='MultiviewKeypoint2dMSEHandler',
            mse_loss=dict(
                type='KeypointMSELoss',
                loss_weight=1.0,
                reduction='sum',
                sigma=100),
            logger=logger),
        dict(
            handler_key='shape_prior',
            type='BetasPriorHandler',
            prior_loss=dict(
                type='ShapePriorLoss', 
                loss_weight=5e-3, 
                reduction='mean'),
            logger=logger),
        dict(
            handler_key='joint_prior',
            type='BodyPosePriorHandler',
            prior_loss=dict(
                type='JointPriorLoss',
                loss_weight=1.0,
                reduction='mean',
                smooth_spine=False,
                smooth_spine_loss_weight=10.0, #10.0
                use_full_body=True,
                lock_foot=True,
                lock_foot_loss_weight=3.0,
                lock_apose_spine=True,
                lock_apose_spine_loss_weight=1.0),
            logger=logger),
        dict(
            handler_key='smooth_joint',
            type='BodyPosePriorHandler',
            prior_loss=dict(
                type='SmoothJointLoss',
                loss_weight=1.0,
                reduction='mean',
                loss_func='L2'),
            logger=logger),
        dict(
            handler_key='smooth_pelvis',
            type='GlobalPosePriorHandler',
            prior_loss=dict(
                type='SmoothPelvisLoss',
                loss_weight=1.0,
                reduction='mean'),
            logger=logger),
        dict(
            handler_key='pose_reg',
            type='BodyPosePriorHandler',
            prior_loss=dict(
                type='PoseRegLoss', loss_weight=0.001, reduction='mean'),
            logger=logger),
        dict(
            handler_key='keypoints3d_limb_len',
            type='Keypoint3dLimbLenHandler',
            loss=dict(
                type='LimbLengthLoss',
                convention='smplx',
                loss_weight=1.0,
                reduction='mean'),
            logger=logger),
    ],
    stages=[
        # # stage 0 betas
        # dict(
        #     n_iter=0, # 10
        #     ftol=1e-4,
        #     fit_global_orient=False,
        #     fit_transl=False,
        #     fit_body_pose=False,
        #     fit_betas=False,
        #     fit_left_hand_pose=False,
        #     fit_right_hand_pose=False,
        #     fit_expression=False,
        #     fit_jaw_pose=False,
        #     fit_leye_pose=False,
        #     fit_reye_pose=False,
        #     keypoints3d_mse_weight=0.0, # 0.0
        #     keypoints2d_mse_weight=0.0,
        #     keypoints3d_limb_len_weight=20., # 5. 20.
        #     shape_prior_weight=5e-3, # 5e-3
        #     joint_prior_weight=0.0,
        #     smooth_joint_weight=0.0,
        #     pose_reg_weight=0.0,
        #     pose_prior_weight=0.0),
        # stage 1 global orient, transl, betas
        dict(
            n_iter=50, # 50
            ftol=1e-4,
            fit_global_orient=True,
            fit_transl=True,
            fit_body_pose=False,
            fit_betas=False,
            fit_left_hand_pose=False,
            fit_right_hand_pose=False,
            fit_expression=False,
            fit_jaw_pose=False,
            fit_leye_pose=False,
            fit_reye_pose=False,
            keypoints3d_mse_weight=1.0,
            keypoints2d_mse_weight=0.0,
            keypoints3d_limb_len_weight=0.5, # 0.5
            shape_prior_weight=0.1,
            joint_prior_weight=0.0,
            smooth_joint_weight=0.0,
            smooth_pelvis_weight=0.0,
            pose_reg_weight=0.0,
            pose_prior_weight=0.0,
            use_shoulder_hip_only=True),
        # # stage 2a pose: fit a general pose # for bad keypoints, skip this stage
        dict(
            n_iter=120, # 120
            ftol=1e-4,
            fit_global_orient=True,
            fit_transl=True,
            fit_body_pose=True,
            fit_betas=False,
            fit_left_hand_pose=True, # True
            fit_right_hand_pose=True, # True
            fit_expression=False,
            fit_jaw_pose=False,
            fit_leye_pose=False,
            fit_reye_pose=False,
            keypoints3d_mse_weight=10.0, #10.0, # 50. original: 1.
            keypoints2d_mse_weight=0.0,
            keypoints3d_limb_len_weight=0.0,
            shape_prior_weight=0.05,
            joint_prior_weight=0.1,  # 0.1
            smooth_joint_weight=1., # 1.
            smooth_pelvis_weight=5.0,
            pose_reg_weight=0.001, # 0.001
            pose_prior_weight=0.0,
            shoulder_weight=1.5, #1.5
            hip_weight=1.5, # 1.5
            body_weight=1.5,
            hand_weight=0.5, # 0.5
            face_weight=0.5,
            foot_weight=1.0, #1.0
            use_shoulder_hip_only=False),
    ],
)
cam_pre_selector = None
cam_selector = None
final_selectors = None
kps3d_optimizers = None

kps3d_optimizers = [ 
    dict(type='PriorConstraint',
        verbose=verbose, 
        logger=logger,
        bbox_size=[2.0,3.0,2.0],
        stand_only=False, # True
        ground_norm=[0.0,-1.0,0.0],
        standing_thr=0.0,
        use_limb_length=True,
        n_max_frame=15,
        diff_thr=0.3, 
        update_mask=False),
    ]
kps3d_optimizers = [ 
    dict(type='NanInterpolation', 
        verbose=verbose, 
        logger=logger),
    ]