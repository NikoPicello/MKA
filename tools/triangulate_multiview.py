...
import sys 
zoehuman_path = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.append(zoehuman_path)
...
def main():
    ...
    img_height = None 
    total_frames = None 
    for video_fn, cam_info in cam_dict.items():
        ...
        if img_width is None or img_height is None:
            ...
            cap.release()
        video_path_arr.append(video_path)
        
        kpt2d_path = os.path.join(args.kpt2d_dir, f"{video_name}_res.npy")
        kpt2d_path_arr.append(kpt2d_path)

        cam_parameter = CameraParameter(name="%02d" % cidx, H=img_height, W=img_width)
        cam_parameter.load_camera_gt(cam_info)
        cam_para_list.append(cam_parameter) 
        cidx += 1       

    out_wo_opt_npy_path = os.path.join(args.out_dir, "no_optim_kpt3d.npz")
    out_w_opt_npy_path = os.path.join(args.out_dir, "optim_kpt3d.npz")
    
    not_valid_arr = []
    kpt2d_all_list = []
    human_data_list = []        
    for ci in range(cam_len):
        cam_str = "%02d" % ci 
        not_valid_arr.append([])
        kpt2d_list = []

        kpt2d_path = kpt2d_path_arr[ci]
        kpt2d_arr = np.load(kpt2d_path, allow_pickle=True)
        frame_len = len(kpt2d_arr)
        print("=======", ci, frame_len, flush=True)
        ...
        for fi in range(total_frames):
            if "kpt2d" in kpt2d_arr[fi]:
                ...
                print(f"=== fill kpt2d {cam_str}_{fi:04d} with previous", flush=True)
                kpt2d_list.append(prev_kpt2d)
            else:
                print(f"=== failed to load kpt2d in {cam_str}_{fi:04d}", flush=True)
                not_valid_arr[ci].append(fi)
                kpt2d_list.append(invalid_kpt2d)

        kpt2d_list = np.stack(kpt2d_list, axis=0)
        kpt2d_all_list.append(kpt2d_list)

    invalid_idx_arr = []
    for ci in range(cam_len):
        invalid_idx_arr += not_valid_arr[ci]
    ...
    for ci in range(cam_len):
        cur_kpt2d = kpt2d_all_list[ci].copy()
        filtered_cur_kpt2d = cur_kpt2d[unique_mask]
        print(f"cam {ci} after filter", filtered_cur_kpt2d.shape, flush=True)
        ...
        human_data_list.append(kpt_dict)
    ...
    keypoints3d_optim = []
    frame_num = human_data_list[0]["keypoints2d"].shape[0]
    interval = 100
    for _fi in trange(0, frame_num, interval):
        ...
        for ci in range(cam_len):
            cur_dict = {
                'keypoints2d': human_data_list[ci]["keypoints2d"][start_fi:end_fi],
                'keypoints2d_mask': kpt2d_mask,
                'keypoints2d_convention': 'smplx'
            }
