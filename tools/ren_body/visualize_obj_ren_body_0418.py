...
def rasterize(
    ...
    for t in front:
        #       [coeffb]*[b-a]
        # [u-a]=[coeffc]*[c-a] ==> u = coeffb * b + coeffc * c + (1-coeffb-coeffc) * a # noqa: E501
        A = np.concatenate(
        ...
        # Texture coloring
            ...
            # c_ = torch.nn.functional.grid_sample(tex, tex_uv_).numpy().reshape(3,-1).T # noqa: E501
...
def compute_normal(vertices, faces):
    # Create a zeroed array with the same type
    # and shape as our vertices i.e., per vertex normal
    ...
    # The cool part, we can actually add the normals
    # through an indexed view of our (zeroed) per vertex normal array.
    norm[faces[:, 0]] += n
    norm[faces[:, 1]] += n
    norm[faces[:, 2]] += n
    normalize_v3(norm)
...
def load_obj_mesh(mesh_file,
    ...
    if with_texture and with_normal:
        ...
        if with_texture_image:
            return vertices, faces, norms, face_normals, uvs, face_uvs, texture_image  # noqa: E501
        else:
...
def main(args):
    # check output path
    exist_result = check_path_existence(args.vis_dir, 'dir')
    ...
    K = cam_dict[args.cam_parameters_key]['K'].copy().reshape(3, 3)
    c2w = cam_dict[args.cam_parameters_key]['RT'].copy().reshape(4, 4)
    dist = cam_dict[args.cam_parameters_key]['D'].copy().reshape(-1)
    obj_names = sorted(os.listdir(args.obj_dir))
    frame_names = sorted(os.listdir(args.background_dir))
    height = 1080
    width = 1920
    # downsample rate, assign a small number to speedup
    ms = 0.25
    K[:2] *= ms
    for index, obj_name in tqdm(enumerate(obj_names)):
        obj_path = os.path.join(args.obj_dir, obj_name)
        verts, faces, normal, _ = load_obj_mesh(obj_path, with_normal=True)
        if index < len(frame_names):
            frame_path = os.path.join(args.background_dir, frame_names[index])
            canvas = cv2.imread(frame_path)
            height = canvas.shape[0]
            width = canvas.shape[1]
        else:
            canvas = np.ones(shape=[height, width, 3], dtype=np.uint8)
        canvas = cv2.resize(canvas, (int(width * ms), int(height * ms)))
        img, _ = rasterize(
            verts,
            faces, (canvas.shape[0], canvas.shape[1]),
            norm=normal,
            img=canvas,
            K=K,
            dist=dist,
        ...
        cv2.imwrite(os.path.join(args.vis_dir, f'{index:06d}.jpg'), img)
    return 0
...
def setup_parser():
    ...
    parser.add_argument(
        '--obj_dir',
        type=str_or_empty,
        help='Path to the meshes directory.',
        default='')
    parser.add_argument(
        '--background_dir',
        type=str_or_empty,
        help='Path to the background image directory.',
        default='')
    parser.add_argument(
        '--annots_path',
        type=str,
    ...
    parser.add_argument(
        '--cam_parameters_key',
        type=str,
        help='Key of the selected camera parameter.',
        default='25')
    # output args
    parser.add_argument(
        '--vis_dir',
        type=str,
        help='Path to the output image dir.',
