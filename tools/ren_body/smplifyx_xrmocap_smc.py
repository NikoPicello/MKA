...
sys.path.append('/nvme/lufan/Projects/zoehuman/tools')
sys.path.append('/nvme/lufan/Projects/zoehuman/xrmocap')
...
def parse_args():
    ...
    parser.add_argument('--smc_id', type=str, default=None)
...
def main():
    ...
    args = parse_args()
    SMCDIR = '/nvme/lufan/data/renbody'
    SMCNAME = f'{args.smc_id}.smc'
    smc_fn = os.path.join(SMCDIR, SMCNAME)
    smc_reader = SMCReader(smc_fn)
    print('Gender ==================> ', smc_reader.actor_info['gender'])
    if args.finetune:
    ...
    estimator_config.smplify.body_model.update(dict(gender = smc_reader.actor_info['gender']))
    ...
    # read pose3d and convert to kps3d
        ...
        if args.kps3d_file == 'None':
            kpts_3d = smc_reader.get_Keypoints3d()
            kpts_3d_dict = {}
            mask = np.load('/nvme/lufan/ckpts/renbody/test/zoehuman_ren_body_full_prev/20220825/dongxinyao_f/dongxinyao_yf2_dz7/smplx_xrmocap/human_data_optimized_keypoints3d.npz')
            mask = mask['keypoints3d_mask'][0]
            kpts_3d_dict['keypoints3d'] = kpts_3d[...,:3]
            # kpts_3d_dict['keypoints3d_mask'] = kpts_3d[0, ...,-1]
            kpts_3d_dict['keypoints3d_mask'] = mask
            # print(kpts_3d_dict['keypoints3d'].shape, kpts_3d_dict['keypoints3d_mask'].shape)
            kpts_3d_dict['convention'] = 'human_data'
            # human_data = HumanData.fromfile(kps3d_file)
            human_data = HumanData.fromdict(kpts_3d_dict)
            logger.info(f"Loader keypoints3D from {smc_fn}")
        else:
            logger.info(f"Loading keypoints 3D from old pipeline: {args.kps3d_file}")
    ...
    if mview_sp_smpl_estimator.kps3d_optimizers is not None:
        ...
        nan_interp = np.ones_like(convert_keypoints3d['keypoints'][...,:1]) * np.nan
        convert_keypoints3d['keypoints'] = np.concatenate([convert_keypoints3d['keypoints'], nan_interp], axis=-1)
    # convert_keypoints3d['keypoints'] = convert_keypoints3d
    ...
    if use_original:
        smpl_dict = smc_reader.get_SMPLx()
        fullpose = smpl_dict['fullpose']
        n_frame = fullpose.shape[0]
        init_smpl_dict['global_orient'] = fullpose[:, 0].reshape(n_frame, 3)
        init_smpl_dict['body_pose'] = fullpose[:, 1:22].reshape(n_frame, 63)
        init_smpl_dict['jaw_pose'] = fullpose[:, 22].reshape(n_frame, 3)
        init_smpl_dict['leye_pose'] = fullpose[:, 23].reshape(n_frame, 3)
        init_smpl_dict['reye_pose'] = fullpose[:, 24].reshape(n_frame, 3)
        init_smpl_dict['left_hand_pose'] = fullpose[:, 25:40].reshape(n_frame, 45)
        init_smpl_dict['right_hand_pose'] = fullpose[:, 40:55].reshape(n_frame, 45)
        init_smpl_dict['expression'] = smpl_dict['expression']
        init_smpl_dict['transl'] = smpl_dict['transl']
        init_smpl_dict['betas'] = smpl_dict['betas']
        init_smpl_dict['scale'] = smpl_dict['scale']
        for key in init_smpl_dict.keys():
            if key != 'scale':
                init_smpl_dict[key] = torch.from_numpy(init_smpl_dict[key]).float()

    if args.init_smpl_dir is not None and 'apose' not in args.sequence:

        # keys = ['betas'] # TODO only betas for now
        # keys = ['betas', 'body_pose'] # TODO only betas for now
        keys = ['betas']
        # keys = ['betas', 'transl', 'body_pose', 'global_orient', 'jaw_pose', 'leye_pose', 'reye_pose', 'left_hand_pose', 'right_hand_pose']
        # keys = ['betas','transl','global_orient']
        
        # name, yf = args.sequence.split("_")[-3:-1] # sequence format: name_fy1_dz1
        ...
        # init_smpl_dir = os.path.join(args.init_smpl_dir, f'{stem}_{args.model}.npz')
        init_smpl_dir = os.path.join(args.init_smpl_dir, f'human_data_tri_smplx.npz')
        if init_smpl_dir.startswith('s3://'):
            init_smpl_f = io.BytesIO(client.get(init_smpl_dir))
            ...
            apose_dict = dict(np.load(init_smpl_dir, allow_pickle=True))
        
        apose_dict_init = apose_dict.copy()
        print(apose_dict.keys())
        ...
        if 'fullpose' in apose_dict_init.keys():
            ...
            # for key in apose_dict.keys():
            #     print(key, apose_dict[key].shape)
        ...
        #     print(key, apose_dict[key].shape)
        # print(apose_dict.keys())
        # print(apose_dict['fullpose'].shape)
        for key in keys:
            # if key == 'gender':
            #     continue
    ...
    # print(init_smpl_dict['betas'])
    
    # if args.finetune:
    #     init_smpl_dict = {k:v[start_t:end_t].reshape(end_t - start_t, *v.shape[1:]) for k, v in init_smpl_dict.items()}
        
    # print(init_smpl_dict.keys())
    # for key in init_smpl_dict.keys():
    #     print(key, init_smpl_dict[key].shape)
    # print(convert_keypoints3d['keypoints'].shape)
    # print(init_smpl_dict['body_pose'].shape)
            
    # print('convert keypoints3d: ', convert_keypoints3d['keypoints'].shape)
    # print(init_smpl_dict.keys())
    ...
    smpl_data.set_gender(smc_reader.actor_info['gender'])
    ...
    for view in args.view_idxs: # 10, 25
        ...
        if n_frame == 0:
            ...
            raise ValueError
        image_array = smc_reader.get_img('Camera_5mp',view,'color',disable_tqdm=True)
        image_array_tensor = torch.from_numpy(image_array).float()
        image_array_tensor = image_array_tensor.permute(0,3,1,2).contiguous()
        H = image_array_tensor.shape[2]
        W = image_array_tensor.shape[3]
        image_array_tensor = F.interpolate(image_array_tensor, size=(int(H//render_scale), int(W//render_scale)))
        image_array_tensor = image_array_tensor.permute(0,2,3,1).contiguous()
        image_array = image_array_tensor.numpy()
        src_image = image_array
        n_frame = image_array.shape[0]

        # load camera
        camera_parameter = FisheyeCameraParameter(name=view)
        # K, R, T, dist_coeff_k, dist_coeff_p, dist_coeff_dict = \
        #     get_camera_param_from_annots(args.annots_file, view)
        calib = smc_reader.get_Calibration_all()['Camera_5mp'][view]
        calib['K'] = calib['K'] / render_scale
        print(calib['K'])
        calib['K'][2,2] = 1.

        K = calib['K']
        R = calib['R']
        D = calib['D'] # k1, k2, p1, p2, k3
        RT = calib['RT']
        R = RT[:3, :3]
        T = RT[:3, 3]
        extrinsic = calib['RT']
