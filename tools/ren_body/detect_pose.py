...
def main(args):
    ...
    for view in range(args.pid * args.num_proc,
                      (args.pid + 1) * args.num_proc):

        view_str = '%02d' % int(view)
        image_dir = os.path.join(args.views_dir, view_str)
...
def setup_parser():
    ...
    parser.add_argument(
        ...
        default='video')
    parser.add_argument(
        '--num_proc', type=int, help='Number of videos per process', default=1)
    parser.add_argument('--pid', type=int, help='Process ID', default=1)
