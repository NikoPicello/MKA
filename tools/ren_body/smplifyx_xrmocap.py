...
sys.path.append('/mnt/petrelfs/luohuiwen/renbody/zip/zoehuman/tools')
sys.path.append('/mnt/petrelfs/luohuiwen/renbody/zip/zoehuman/xrmocap')
...
def get_image_array(background_dir, view):
    ...
    if frame_dir.startswith('s3://'):
        ...
        view_dir = os.path.join(background_dir, view)
        filenames = sorted(os.listdir(view_dir))
        n_frame = len(filenames)
        image_list = []
        for img_fn in filenames:
            img_path = os.path.join(view_dir, img_fn)
