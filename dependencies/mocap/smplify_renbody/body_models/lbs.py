...
def find_dynamic_lmk_idx_and_bcoords(vertices,
    ...
    rel_rot_mat = torch.eye(
        3, device=vertices.device, dtype=dtype).unsqueeze_(dim=0)
...
def batch_dqs_blending(A, W, Vs):
    ...
    Rs = A[:, :3, :3]
    ws = torch.sqrt(
        torch.clamp(Rs[:, 0, 0] + Rs[:, 1, 1] + Rs[:, 2, 2] + 1.,
                    min=1.e-6)) / 2.
    ...
    b0 = W.unsqueeze(-2) @ torch.cat(
        [ws[:, None], xs[:, None], ys[:, None], zs[:, None]], dim=-1).reshape(
            Bnum, 1, Jnum, 4)  # B,V,J,4
    be = W.unsqueeze(-2) @ torch.cat(
        [vDw[:, None], vDx[:, None], vDy[:, None], vDz[:, None]],
    ...
    Vs = Vs + 2. * b0[:,
                      1:].cross(b0[:, 1:].cross(Vs) + b0[:, :1] * Vs) + 2. * (
                          b0[:, :1] * be[:, 1:] - be[:, :1] * b0[:, 1:] +
                          b0[:, 1:].cross(be[:, 1:]))
