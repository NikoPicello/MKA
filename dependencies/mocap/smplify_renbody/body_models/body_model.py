...
from .lbs import batch_rodrigues, dqs, lbs
...
def load_bodydata(model_type, model_path, gender):
    if osp.isdir(model_path):
        model_fn = '{}_{}.{ext}'.format(
            model_type.upper(), gender.upper(), ext='pkl')
...
class SMPLlayer(nn.Module):
    ...
    def __init__(self,
        ...
        if use_pose_blending:
            ...
            data['posedirs'] = np.reshape(data['posedirs'],
                                          [-1, num_pose_basis]).T
        ...
        if self.use_shape_blending:
            if self.model_type == 'smplx':
                ...
                self.num_shapes = 10
                self.shapedirs = self.shapedirs[:, :, :self.num_shapes +
        ...
        if regressor_path is not None and use_joints:
            ...
            j_J_regressor = torch.zeros(
                self.J_regressor.shape[0], X_regressor.shape[0], device=device)
        ...
        if self.model_type == 'smplh':
            ...
            for key in ['LEFT', 'RIGHT']:
                left_file = join(kwargs['mano_path'],
                                 'MANO_{}.pkl'.format(key))
                with open(left_file, 'rb') as f:
                    data = pickle.load(f, encoding='latin1')
                val = to_tensor(
                    to_np(data['hands_mean'].reshape(1, -1)), dtype=dtype)
                self.register_buffer('mHandsMean' + key[0], val)
                val = to_tensor(
                    to_np(data['hands_components'][:self.num_pca_comps, :]),
                    dtype=dtype)
            ...
            for key in ['Ll', 'Rr']:
                val = to_tensor(
                    to_np(data['hands_mean' + key[1]].reshape(1, -1)),
                    dtype=dtype)
                self.register_buffer('mHandsMean' + key[0], val)
                val = to_tensor(
                    to_np(data['hands_components' +
                               key[1]][:self.num_pca_comps, :]),
                    dtype=dtype)
                self.register_buffer('mHandsComponents' + key[0], val)
            self.use_pca = True
    ...
    def extend_pose(self, poses):
        ...
        if self.model_type not in ['smplh', 'smplx', 'mano']:
            return poses
        elif self.model_type == 'smplh' and poses.shape[
                -1] == 156 and self.use_flat_mean:
            return poses
        elif self.model_type == 'smplx' and poses.shape[
                -1] == 165 and self.use_flat_mean:
            return poses
        elif self.model_type == 'mano' and poses.shape[
                -1] == 48 and self.use_flat_mean:
            return poses
        # skip mano
        if self.model_type == 'mano':
            poses_hand = self.extend_hand(poses[..., 3:], self.use_pca,
                                          self.use_flat_mean,
                                          self.mHandsComponents,
                                          self.mHandsMean)
        ...
        poses_rh = poses[:, NUM_BODYJOINTS + NUM_HANDJOINTS:NUM_BODYJOINTS +
                         NUM_HANDJOINTS * 2]
        ...
        if self.model_type == 'smplh':
            poses = torch.cat([poses[:, :NUM_BODYJOINTS], poses_lh, poses_rh],
                              dim=1)
        elif self.model_type == 'smplx':
            ...
            poses = torch.cat(
                [poses[:, :NUM_BODYJOINTS], poses_head, poses_lh, poses_rh],
                dim=1)
        return poses
    ...
    def convert_from_standard_smpl(self,
                                   poses,
                                   shapes,
                                   Rh=None,
                                   Th=None,
    ...
    def forward(self,
        ...
        Args:
            poses (n, 72). smplx (n, 87) = 22*3(body) + 6*2(two hands) + 9(eyes&jaw)
            shapes (n, 10)
            Rh (n, 3): global orientation
            Th (n, 3): global translation
            return_verts (bool, optional): if True return (6890, 3).
                Defaults to False.
        """
        if 'torch' not in str(type(poses)):
            dtype, device = self.dtype, self.device
