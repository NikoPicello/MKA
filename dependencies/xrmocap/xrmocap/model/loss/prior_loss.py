...
class JointPriorLoss(torch.nn.Module):
    ...
    def forward(self,
        ...
        if self.smooth_spine:
            ...
            # norm_spine1 = spine1/torch.norm(spine1,p=2,dim=-1,keepdim=True)
            # norm_spine2 = spine2/torch.norm(spine2,p=2,dim=-1,keepdim=True)
            # norm_spine3 = spine3/torch.norm(spine3,p=2,dim=-1,keepdim=True)
            # cos_12 = torch.einsum('bn,bn->b', -norm_spine1, norm_spine2)
            # cos_23 = torch.einsum('bn,bn->b', -norm_spine2, norm_spine3)
            
            # smooth_spine_loss_12 = (torch.exp(F.relu(cos_12 + np.cos(np.deg2rad(30)) )) -
            #                         1).pow(2).repeat(1,3) * self.smooth_spine_loss_weight/3.
            # smooth_spine_loss_23 = (torch.exp(F.relu(cos_23 + np.cos(np.deg2rad(30)) )) -
            #                         1).pow(2).repeat(1,3) * self.smooth_spine_loss_weight/3.
            ...
            # smooth_spine_loss_12 = F.relu(-spine1 * spine2).pow(2) * self.smooth_spine_loss_weight
            # smooth_spine_loss_23 = F.relu(-spine2 * spine3).pow(2) * self.smooth_spine_loss_weight

            # print("=== smooth_spine", smooth_spine_loss_12.shape, smooth_spine_loss_12.sum(), smooth_spine_loss_23.sum(), flush=True)
        ...
        for idx, weight in enumerate(weights):
            ...
            lower_loss = (
                torch.exp(F.relu(lower_limits - pred_pose)) - 
                1).pow(2)
            upper_loss = (
                torch.exp(F.relu(pred_pose - upper_limits)) - 
                1).pow(2)
            loss = (lower_loss + upper_loss).view(
                body_pose.shape[0], -1) # (n, 3)
...
class SmoothPelvisLoss(torch.nn.Module):
